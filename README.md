...
- Only tables Obsidian rendered from the note are managed. Other plugins mount their own UI inside a view — Editing Toolbar, for instance, inserts a toolbar containing real `<table>` colour pickers into `.markdown-source-view` — so tables outside `.cm-content`/`.markdown-preview-sizer` are ignored, both when scanning and when watching for changes.
